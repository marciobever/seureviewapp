import { GoogleGenAI, Type, FunctionDeclaration, GoogleGenAI as GoogleGenAIVideo } from "@google/genai";
...
// Cria o cliente SOMENTE se tiver chave
const ai = apiKey ? new GoogleGenAI({ apiKey }) : null;

const requireAI = () => {
  if (!apiKey || !ai) {
    throw new Error("A chave da IA (VITE_GEMINI_API_KEY) não está configurada.");
  }
  return ai;
};
...
export const generateReelsVideo = async (prompt: string): Promise<string> => {
  ...
  try {
    const aiVideo = new GoogleGenAIVideo({ apiKey });
