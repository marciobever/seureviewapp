...
// Lê a chave exposta pelo Vite no front-end
const API_KEY = (import.meta as any)?.env?.VITE_GEMINI_API_KEY as string | undefined;

// Não derruba o app se não tiver chave; só avisa
if (!API_KEY) {
  console.warn(
    "VITE_GEMINI_API_KEY não está definida. Os recursos de IA ficarão limitados até você configurar essa variável no ambiente."
  );
}

// Cliente principal de texto/imagem
const ai = API_KEY ? new GoogleGenAI({ apiKey: API_KEY }) : null;

// Helper para garantir que só usamos o cliente quando existir
const requireAI = () => {
  if (!ai) {
    throw new Error(
      "A chave VITE_GEMINI_API_KEY não está configurada no ambiente. " +
      "Defina essa variável no Coolify para usar os geradores de IA."
    );
  }
  return ai;
};
...
// =================================================
// Etapa 1: Busca uma lista de opções de produtos
// =================================================
...
// =================================================
// Etapa 2: Gera conteúdo para um produto selecionado
// =================================================
...
const requireVideoClient = () => {
  if (!API_KEY) {
    throw new Error(
      "A chave VITE_GEMINI_API_KEY não está configurada. Configure-a para usar a geração de vídeos."
    );
  }
  return new GoogleGenAIVideo({ apiKey: API_KEY });
};

export const generateReelsVideo = async (prompt: string): Promise<string> => {
  try {
    const aiVideo = requireVideoClient();
...
// =================================================
// Geração de imagem de marketing
// =================================================

export const generateMarketingImage = async (prompt: string): Promise<string> => {
  try {
    const response = await requireAI().models.generateImages({
...
// =================================================
// Sugestões de otimização de post
// =================================================
