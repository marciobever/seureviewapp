    ...
    <div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="w-3 h-3 flex-shrink-0" title={statusTitle}>
          <div className={`w-full h-full rounded-full transition-colors ${statusColor}`} />
        </div>
      </div>
    </div>
  );
...
export const ApiKeysPage: React.FC = () => {
  ...
  const validateKey = (name: string, value: string): ValidationStatus => {
    ...
    switch (name) {
      ...
      case 'aliexpress':
      case 'shein':
      case 'n8nApiKey':                 // <- valida n8n também
  ...
  const stripeBackendCode = `// Exemplo de backend (Node.js/Express) para criar uma Sessão de Checkout
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="w-12 h-12 border-4 border-t-orange-500 border-slate-700 rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in">
      ...
      <p className="text-gray-400 mb-8">
        Conecte suas contas de afiliado e seu gateway de pagamento. As chaves são salvas de forma segura no seu perfil.
      </p>

      {/* Stripe Section (igual ao seu, encurtei o código de exemplo) */}
      <div className="mb-10">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-8 space-y-6">
          ...
          <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-6">
            ...
            <p className="text-gray-400 mb-4 text-sm">
              Para processar pagamentos de forma segura, a lógica de criação de checkout deve rodar no seu servidor.
            </p>
      ...
      <form onSubmit={handleSubmit}>
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-8">
          <div className="space-y-6">
            <ValidatedInputField
              label="API Key - Mercado Livre"
              id="mercadoLivre"
              value={keys.mercadoLivre}
              onChange={handleChange}
              placeholder="Cole sua chave da API aqui"
              status={validation.mercadoLivre || 'empty'}
            />
            <ValidatedInputField
              label="AppID - Shopee"
              id="shopeeAppId"
              value={keys.shopeeAppId}
              onChange={handleChange}
              placeholder="Cole seu AppID da Shopee aqui"
              type="text"
              status={validation.shopeeAppId || 'empty'}
            />
            <ValidatedInputField
              label="Senha da API - Shopee"
              id="shopeePassword"
              value={keys.shopeePassword}
              onChange={handleChange}
              placeholder="Cole sua senha da API aqui"
              status={validation.shopeePassword || 'empty'}
            />
            <ValidatedInputField
              label="API Key - Amazon Associados"
              id="amazon"
              value={keys.amazon}
              onChange={handleChange}
              placeholder="Cole sua chave da API aqui"
              status={validation.amazon || 'empty'}
            />
            <ValidatedInputField
              label="API Key - Temu"
              id="temu"
              ...
              onChange={handleChange}
              placeholder="Cole sua chave da API aqui"
              status={validation.temu || 'empty'}
            />
            <ValidatedInputField
              label="API Key - Aliexpress"
              id="aliexpress"
              value={keys.aliexpress}
              onChange={handleChange}
              placeholder="Cole sua chave da API aqui"
              status={validation.aliexpress || 'empty'}
            />
            <ValidatedInputField
              label="API Key - Shein"
              id="shein"
              ...
              onChange={handleChange}
              placeholder="Cole sua chave da API aqui"
              status={validation.shein || 'empty'}
            />
            <ValidatedInputField
              label="API Key - n8n (para webhooks protegidos)"
              id="n8nApiKey"
              value={keys.n8nApiKey}
              onChange={handleChange}
              placeholder="Cole a x-api-key do n8n aqui"
              status={validation.n8nApiKey || 'empty'}
              type="text"
            />
          </div>
          ...
          <div className="pt-6 mt-6 border-t border-slate-700 text-right">
            ...
            <button
              type="submit"
              disabled={isSaving}
              className="px-6 py-2 bg-orange-600 text-white font-semibold rounded-lg shadow-md hover:bg-orange-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
